...
st.caption("HIPAA-Safe Mode: Uses Patient IDs and Medication Names/Rx Numbers. No PHI stored.")

# Initialize session state with medication lists
        ...
        if p["Status"] == "Active":
            try:
                ...
                worklist.append({
                    ...
                    "Medications": p.get("Medications", "None"),
                    "Status": "Pending Call"
                })
            except ValueError:
                pass
...
# ---------------------------------------------------------
with tab2:
    st.subheader("Register Patient ID & Medications")
    ...
    with st.form("add_patient_form"):
        ...
        # New Medication Entry Field
        meds_input = st.text_area(
            "Medication List / Rx Numbers (comma-separated)",
            placeholder="e.g., Lisinopril 20mg, Metformin 1000mg, Atorvastatin 40mg"
        )
        ...
        if submitted:
            if new_id:
                st.session_state.patients.append({
                    ...
                    "Cycle": cycle,
                    "Medications": meds_input if meds_input else "None listed",
                    "Status": "Active"
                })
